...
_NON_RETRIABLE_RE = re.compile(
    # HTTP status codes: match when preceded by HTTP context, whitespace/punctuation,
    # or at the very start of the string. Avoids false positives on "400ms", "401k", etc.
    r"(?:^|http(?:/\S+)?\s+|status\s*(?:code\s*)?|code\s+|[:\[(\s])(400|401|403|404)\b"
