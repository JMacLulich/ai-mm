...
REVIEW_SYSTEM_PROMPTS = {
    ...
    "testing": (
        ...
        "REAL services. Every mock requires justification. Do not recommend adding more mocks "
        "when the missing proof is an integration or E2E path.\n\n"
        "THE MOCK TRAP (HIGH PRIORITY): Pipeline tests that patch token refreshers, handlers, "
        "HTTP clients, DB sessions, queues, or pipeline factories can pass while the real "
        "implementation is completely broken. Treat this as a review finding, not a style "
        "nit. Use this exact test when auditing: 'Could this suite pass if the real pipeline "
        "wiring, credentials, migrations, filesystem paths, service response shape, or network "
        "call is broken?' If yes, say so directly and request the missing real-path test.\n\n"
        ...
        "aren't tested\n"
        "- Wiring Mirage: test patches the factory/handler/client that production code must "
        "actually assemble\n"
        "- Credential Mirage: token/API-key/auth refresh path is mocked so real credential "
        "loading and refresh behavior is never executed\n"
        "For each mock found, ask: 'Could this test pass even if the real "
        "implementation is broken?' "
        "If yes, flag it and describe the real test that would fail before the fix.\n\n"
        ...
        "If the only proposed fix is another mock, reject it and ask for real-service coverage."
    ),
}
